...
public:
	...
	TextQueryResult(string a, vector<string> b, shared_ptr<set<vector<string>::size_type>> c)
...
TextQuery::TextQuery(ifstream &is):Text(new vector<string>(),DebugDelete()) {
	string line;
	while (getline(is, line))
	{
		...
		while (strstream >> word)
		{
			...
			if (!info)
			{
				info.reset(new set<vector<string>::size_type>);
			}
			info->insert(lineNO);
		}
	}
